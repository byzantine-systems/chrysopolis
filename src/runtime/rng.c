...
void rng_reseed_from(const rng_provider_t *provider) {
  if (!rng_reseed_applies(g_drbg_ready != 0, provider)) {
    return;
  }
  uint8_t raw[64];
...
/* The FNV-1a 64-bit prime, also mixed into the fallback seed below. */
...
static constexpr size_t jitter_samples = 1024;
 ...
 * mix. No whitening here, HMAC-DRBG conditions whatever we hand it. The
 * counter reads and the scratch store live here; the folding and the health
 * rule are rng_select.c.
 */
static size_t jitter_collect(uint8_t *buf, size_t len) {
  rng_jitter_state state = {};
  rng_jitter_init(&state, read_cntpct());

  uint64_t prev = read_cntpct();
  for (size_t i = 0; i < jitter_samples; i++) {
    jitter_scratch[i & 63] ^= rng_jitter_scratch_byte(&state, i);
    __asm__ volatile("" ::: "memory");
    const uint64_t now = read_cntpct();
    rng_jitter_absorb(&state, i, now - prev);
    prev = now;
  }
...
void rng_init(void) {
  uint8_t seed[64];
  size_t seed_len = jitter_provider.collect(seed, sizeof(seed));
  const rng_seed_source source = rng_select_seed(seed_len);
  if (source == rng_seed_source_fallback) {
  ...
  rng_realtime_offset_sec = rng_realtime_offset(off);
