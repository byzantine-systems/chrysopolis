/*
 * Bring-up shims for the beam_server PD (Option 2: in-memory filesystem).
 *
...
/* libc_init_file override: Option C workaround - no fs_server present.
...
/* openat: Option 2 - use in-memory filesystem */
...
/* fstatat: Option 2 - stat in-memory files */
...
void bringup_register_syscalls(void) {
  libc_define_syscall(SYS_openat, bringup_openat);      /* Option 2: memfs */
  libc_define_syscall(SYS_newfstatat, bringup_fstatat); /* Option 2: memfs */
